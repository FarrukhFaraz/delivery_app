...
class AdminPanelController extends Controller
{
    ...
    function dashboard()
    {
        ...
        $restaurant = RestaurantProfile::all()->first();
    ...
    function uploadProfile(Request $request)
    {
        ...
        try {
            ...
            if ($restaurant == null) {
                ...
                if ($request->has('image')) {
                    $restaurant->logo = $logo;
                    if (File::exists('uploads/profile/' . $restaurant->logo)) {
                        File::delete('uploads/profile/' . $restaurant->logo);
                    }
                }
            ...
            if ($request->has('image')) {
                $request->file('image')->move('uploads/profile/', time());
            }
    ...
    function category_created(Request $request)
    {
        ...
        try {
            ...
            $imageFile ='uploads/categories/'. $time . '.' . $extension;

            $request->file('image')->move($imageFile);
            $url = asset($imageFile);
            $category->image = $url;
    ...
    function category_updated(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('image')) {
                ...
                $request->file('image')->move('uploads/categories/', time() . '.' . $extension);

                $category->image = time() . '.' . $extension;
            }
            $result = $category->save();
            if ($result) {
